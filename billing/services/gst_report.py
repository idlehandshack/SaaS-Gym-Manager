...
---------------------------------
Generates a gym-specific GST Sales Report workbook for EnterGYM.

Sheet 1 — GST Sales Report   (all issued invoices in the period, with header, table, totals, footer)
Sheet 2 — GST Summary        (aggregated by GST rate — supports 5%, 12%, 18%, etc.)

Usage:
    from billing.services.gst_report import generate_gstr1_style_report
    from datetime import date

    buf = generate_gstr1_style_report(gym, date(2026, 4, 1), date(2026, 4, 30))
    response = HttpResponse(buf.read(),
                   content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    response['Content-Disposition'] = 'attachment; filename="GST_Report_Apr2026.xlsx"'
    return response
"""
...
TOTALS_FILL    = PatternFill(fill_type='solid', fgColor='D6E4F0')   # light blue
...
FOOTER_FONT    = Font(italic=True, size=9, name='Arial', color='808080')  # gray italic
...
INR_FMT  = '[\u20b9-4009]#,##0.00'   # ₹ symbol with Indian locale hint
PCT_FMT  = '0"%"'                     # e.g. 18%


# ── Helpers ────────────────────────────────────────────────────────────────────

def _set_headers(sheet, headers: list[str], col_widths: list[int], start_col: int = 1):
    """Write a styled header row; col numbers are 1-based."""
    row = sheet.max_row + 1
...
def _style_data_row(sheet, row_num: int, num_cols: int, start_col: int = 1):
...
def _apply_currency_fmt(sheet, row_num: int, col_indices: list[int]):
    """Apply INR number format to specific columns in a row."""
    for col in col_indices:
        sheet.cell(row=row_num, column=col).number_format = INR_FMT


def _merge_write(sheet, row: int, start_col: int, end_col: int,
                 value, font=None, alignment=None):
    """Merge a range of cells in a single row and write a value."""
    sheet.merge_cells(
...
def generate_gstr1_style_report(gym, start_date, end_date) -> BytesIO:
    """
    Returns a BytesIO containing an .xlsx workbook with:
      Sheet 1: GST Sales Report  (invoice-level detail)
      Sheet 2: GST Summary       (aggregated by GST rate)
    """
    # Single queryset — no N+1 queries
    invoices = list(
        Invoice.objects
...
def _build_sales_sheet(wb, gym, invoices, start_date, end_date):
    ...
    num_cols = 11          # total table columns (A–K)
    ...
    # Rows 3-8: Report metadata (label in col A, value in cols B–D)
    # GST fields live on the related GymGSTProfile (gym.gst_profile)
    ...
    for label, value in meta_rows:
        ...
        # merge value columns so it reads cleanly
        sheet.merge_cells(
    ...
    col_widths = [18, 14, 26, 16, 16, 12, 12, 12, 12, 14, 12]

    # Capture header row BEFORE writing it so freeze is always correct,
    # regardless of how many meta rows are added above in the future.
    table_header_row = sheet.max_row + 1
    _set_headers(sheet, headers, col_widths)

    # Freeze everything above the first data row so the header stays visible.
    sheet.freeze_panes = f'A{table_header_row + 1}'

    # currency columns (1-based): Taxable=5, CGST=6, SGST=7, IGST=8, TotalGST=9, Grand=10
    currency_cols = [5, 6, 7, 8, 9, 10]

    # Running totals (Decimal for accuracy)
    ...
    for inv in invoices:
        ...
        _apply_currency_fmt(sheet, row_num, currency_cols)
    ...
    # ── Totals row ─────────────────────────────────────────────────────────────
    sheet.append([])    # empty separator before totals
    ...
    _apply_currency_fmt(sheet, totals_row, currency_cols)
    sheet.row_dimensions[totals_row].height = 20

    # ── Footer ─────────────────────────────────────────────────────────────────
    sheet.append([])
    footer_row1 = sheet.max_row + 1
    _merge_write(
        sheet, row=footer_row1, start_col=1, end_col=num_cols,
        value='This report was generated automatically by EnterGYM.',
        font=FOOTER_FONT,
        alignment=Alignment(horizontal='left'),
    )

    footer_row2 = sheet.max_row + 1
    _merge_write(
        sheet, row=footer_row2, start_col=1, end_col=num_cols,
...
def _build_summary_sheet(wb, gym, invoices, start_date, end_date):
    ...
    # ── Aggregate line items → rate buckets ───────────────────────────────────
    # invoice_ids is a set so each invoice is counted once per rate slab,
    # even when it has multiple line items at the same rate.
    ...
    col_widths = [16, 16, 18, 14, 14, 14, 14]
    ...
    # ── Data rows ─────────────────────────────────────────────────────────────
    # Running totals for the grand-total footer row
    grand_inv_ids = set()
